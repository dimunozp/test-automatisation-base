...
Feature: Create Character from Marvel API
  ...
  Background:
    * def requestBody = read('classpath:../features/marvel_api/createCharacterRequest.json')
  ...
  Scenario: T-API-HU-123-CA3-Create a new character but character already exists
    When method POST
    Then status 400
      And match response == '#object'
